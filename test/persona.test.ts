...
describe('Persona', () => {
  ...
  describe('saveToFile', () => {
    ...
    it('should throw error when trying to write to read-only file', () => {
      ...
      const persona = new Persona('Test', 'Description', 'Template', 1);
      expect(() => persona.saveToFile(testFilePath)).toThrow('Failed to save persona: EPERM: operation not permitted, open');
  ...
  describe('loadFromFile', () => {
    ...
    it('should throw error when trying to read from write-only file', () => {
      ...
      try {
        ...
        expect(() => Persona.loadFromFile(testFilePath)).toThrow('Failed to load persona: Error: EACCES: permission denied, open');
      } finally {
    ...
    it('should throw error for JSON missing required fields', () => {
      ...
      expect(() => Persona.loadFromFile(testFilePath)).toThrow('Invalid persona data in file');
    });
  });
  ...
  describe('render', () => {
    ...
    beforeEach(() => {
      persona = new Persona('Test', 'Description', '{{ greeting }} {{ name }}!', 1);
    });
  ...
  describe('requiredComponents', () => {
    ...
    it('should return variable names case insensitive', () => {
      const persona = new Persona('Test', 'Description', '{{ GREETING }} {{ NAME }}', 1);
      expect(persona.requiredComponents()).toEqual(['greeting', 'name']);
    });
